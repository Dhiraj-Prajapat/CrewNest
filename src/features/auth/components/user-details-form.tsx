...
export default function UserDetailsForm() {
  ...
  const steps: Step[] = [
    ...
    },
    {
      ...
      validation: (value: string) => {
        ...
        if (age < 18)
          return "You can't access our workspace. You must be 18 or older.";
        return null;
      },
    },
    {
      ...
      validation: (value: string) => {
        ...
        if (!phoneRegex.test(value))
          return "Please enter exactly 10 digits only";
        return null;
      },
    },
    {
    ...
    },
    {
      ...
      placeholder:
        "Share your story, interests, goals, or anything you'd like your team to know...",
      type: "textarea",
    ...
    },
    {
      id: "complete",
      question:
        "Thanks for completing your profile! 🎉 We really appreciate you giving your valuable time to us. Welcome to our amazing workspace! 🚀",
      type: "complete",
  ...
  const validateCurrentStep = (): boolean => {
    if (currentStepData.validation) {
      const validationError = currentStepData.validation(
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="bg-[#296a52] rounded-2xl shadow-xl p-8 relative overflow-hidden">
          ...
          >
            <div className="relative">
              <div
                className={`w-26 h-20 bg-gradient-to-br bg-[#296a52]  flex items-center justify-center text-white text-2xl font-bold  transition-all duration-500 ${isAnimating ? "animate-bounce" : ""}`}
              >
          ...
          {/* Question */}
          <div
            className={`text-center mb-8 transition-all duration-500 ${!isTypingComplete ? "opacity-90" : "opacity-100"}`}
          >
                ...
                <div className="relative">
                  ...
                  {currentStepData.type === "select" ? (
                    ...
                    <textarea
                      ...
                      className="w-full pl-12 pr-4 py-4 border-2 bg-[#f2f0EB] border-gray-200 rounded-xl focus:border-primary focus:outline-none transition-all duration-300 resize-none h-32 text-gray-700"
                      autoFocus
                      required
                    />
                  ) : (
                    <input
                      ...
                      onChange={(e) => handleInputChange(e.target.value)}
                      placeholder={currentStepData.placeholder}
                      className="w-full pl-12 pr-4 py-4 border-2 bg-[#f2f0EB] border-gray-200 rounded-xl focus:border-primary focus:outline-none transition-all duration-300 text-gray-700"
                      autoFocus
                      required
                      onKeyPress={(e) =>
                        e.key === "Enter" &&
          ...
          {(currentStep === 0 || currentStep === steps.length - 1) &&
            isTypingComplete && (
              ...
              >
                <button
                  onClick={handleNext}
                  className="px-12 py-4 bg-white text-gray-600 rounded-xl font-semibold text-lg shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300 flex items-center space-x-3 mx-auto"
                >
                  <span>
                    {currentStep === 0 ? "Let's Start!" : "Go to Workspace"}
                  </span>
                  {currentStep === 0 ? (
        ...
        {currentStep > 0 && currentStep < steps.length - 1 && (
          <div className="text-center mt-6">
            <p className="text-gray-100 text-sm animate-pulse">
              🔒 Your information is secure and will only be visible to your
              team members
            </p>
          </div>
        )}
